...
import logging
import argparse
from src.models.model_config import model_cfg  # config the model
...
from src.tools.laplace import laplace_function
...
def tracking(pose_cur, pose, person_id, img_id):
    pose_current = pose_cur.copy()
    pose_pre = pose.copy()

    for i in range(len(pose_pre)):
        if len(pose_current) == 0:
            break
        dist_ave = []
        for j in range(len(pose_current)):
            dist = np.square(pose_current[j] - pose_pre[i])
        ...
        a = get_key(person_id, pose_pre[i][0][0])
        tmp = person_id[a]
        tmp.append(pose_current[cur_id][0][0])
        person_id[a] = tmp
        pose_current.pop(cur_id)

    if len(pose_current) != 0:
        for m in range(len(pose_current)):
            person_id[len(person_id.keys()) + m] = [pose_current[m][0][0]]
...
if __name__ == '__main__':
    ...
    for dataset_idx, dataset_name in enumerate(args.datasets):
        ...
        if dataset_name == 'Shelf':
            ...
            test_range = [i for i in range(0, 200, 1)]
            gt_path = dataset_path
